...
fn score_line(line: &str) -> i64 {
    let items = line.split_whitespace().map(|i| i.parse::<i64>().expect("Not parseable")).collect::<Vec<i64>>();
...
fn score_alternative_lines(items: &Vec<i64>, i: usize) -> i64 {
    ...
    *vec![first_alternative, second_alternative, third_alternative].iter().max().expect("Empty vector")
}
...
mod tests {
    ...
    #[test]
    fn test_calculate() {
        let result = calculate(&(read_to_string("resources/tests/02.txt").expect("Test file not found.")));
        assert_eq!(result, 2);
    }

    #[test]
    fn test_calculate_variant() {
        let result = calculate_variant(&(read_to_string("resources/tests/02.txt").expect("Test file not found.")));
        assert_eq!(result, 7);
    }
}